...
- [ ] Feature deduplication: the scraper extracts near-duplicate features from different pages of a product site (e.g. "Custom vocabulary / glossary" and "Custom vocabulary", "Bring-your-own API keys" and "Bring your own API keys"). `save_features` only deduplicates by exact name match, so these accumulate with every re-scrape. Add a deduplication step, either during `save_features` (via a `claude -p` call to merge semantically identical features) or as a post-extraction cleanup pass. Should also deduplicate against already-implemented features (e.g. "Local offline transcription" remaining when "Local on-device transcription" is implemented).
- [ ] Appshot timeout: `_complete_phase` calls `capture_appshot` which can hang indefinitely if the app is a menu bar app with no main window, or if `run.sh` triggers a long build. Add a timeout to the appshot call so it cannot block the phase-completion flow. Skip gracefully on timeout.
- [ ] Suppress source-file URL scanning on subsequent runs: the file change detection in `_subsequent_run` calls `_analyze_new_files` which calls `scan_files`, but `scan_files` uses the same `_classify_file` logic. Verify the `_SOURCE_EXTS` and `_SOURCE_NAMES` filtering added to `scanner.py` also applies to the `scan_files` path (it should, since both go through `_classify_file`).
- [ ] Scope `_plan_is_complete` and `_complete_phase` to the current phase only: now that PLAN.md is no longer deleted between phases, the file accumulates all phases. `_plan_is_complete()` checks ALL checkboxes in the file, and `_complete_phase` processes ALL checked tasks. Both need to be scoped to only the section between the current phase heading and the next phase heading (or end of file). Without this, completing Phase 2 will re-trigger phase-completion for Phase 1 tasks.
- [ ] Do not generate standalone "add tests" tasks: the mcloop task prompt already instructs every session to "Write unit tests where they make sense," so implementation tasks write their own tests. Dedicated test tasks at the end of a phase are redundant and fail as no-ops because the tests already exist. Remove `generate_plan_test_tasks` from plan generation entirely. If test coverage for a feature matters, include it in the implementation task description (e.g. "Implement X with unit tests for Y and Z").
- [ ] Language-aware test generation: `test_generator.py` generates Python test files and `generate_plan_test_tasks` appends "wire up doc-example tests" tasks regardless of the target project's language. For non-Python projects (Swift, Rust, Go, etc.) these tasks are invalid and waste mcloop retries. The test generator should either generate tests in the target language or skip test generation entirely when the target language is not Python.
- [ ] Skip re-scrape and feature re-extraction when site content is unchanged: `_subsequent_run` unconditionally re-scrapes the product URL (60 pages) and re-runs Claude feature extraction on every invocation, even when entering the next-phase flow immediately after phase completion. The content hashes for each page are already stored in `duplo.json`. Compare new content hashes against stored ones and skip feature re-extraction if nothing changed. When no PLAN.md exists and we are just generating the next phase, the re-scrape from the previous run (minutes ago) is still current, so skip it entirely.
