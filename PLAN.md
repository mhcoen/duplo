...
- [ ] Fix `append_to_bugs_section` boundary scan in `duplo/saver.py`
  - [ ] Function at `duplo/saver.py:1308-1321` ends its `## Bugs` section scan only at the next `## ` (H2) heading, walking past `# ` (H1) phase headings. As a result, when duplo writes a new bug entry into PLAN.md, the entry can land deep inside an unrelated phase (typically Phase 2, before its `## Manual verification` subheading) rather than at the end of the `## Bugs` section. This corrupts PLAN.md structure on every duplo run that touches the bugs section.
  - [ ] Fix: widen the boundary regex in the scan loop so it terminates on either H1 (`^# `) or H2 (`^## `) headings, not just H2. The intent is "end of section," which in this PLAN.md format means "any heading at the same or higher level than `## Bugs`."
