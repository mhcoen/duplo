...
- [x] Audit all callers of _first_run in duplo/main.py
  - [x] Grep the codebase for references to _first_run. Identify every call site. The only known call is in main() dispatch: when .duplo/duplo.json does not exist the code calls _first_run(url=args.url).
  ...
  - [x] Confirm that the migration gate prevents any old-format project from reaching _first_run.

- [x] Remove _first_run and its helper functions from duplo/main.py
  - [x] Delete the _first_run function.
  - [x] Delete _confirm_product (interactive product confirmation prompt, only called by _first_run).
  - [x] Delete _validate_url (interactive URL disambiguation prompt, only called by _first_run).
  - [x] Delete _init_project (saves selections, generates tests, writes CLAUDE.md, builds roadmap for first run, only called by _first_run).
  - [x] For each deletion, audit callers first. If any live code path calls the function, do not delete it.
  - [x] Tests: no test references _first_run, _confirm_product, _validate_url, or _init_project.

- [x] Update the no-subcommand dispatch in main() to remove the _first_run branch
  - [x] After migration check passes: if SPEC.md exists, go to _subsequent_run. If SPEC.md does not exist and .duplo/duplo.json does not exist (fresh directory, not an old project), print a message directing the user to run duplo init and exit 0.
  - [x] Remove the import of ask_preferences from duplo.questioner in main.py (only used by _first_run).
  - [x] Tests: duplo in a fresh directory (no .duplo/, no SPEC.md) prints "run duplo init" message and exits 0; duplo in a directory with SPEC.md proceeds to _subsequent_run.

## BuildPreferences migration

- [x] Move BuildPreferences dataclass from duplo/questioner.py to duplo/build_prefs.py
  - [x] The BuildPreferences dataclass is used by _prefs_from_dict and _load_preferences in main.py, both called from _subsequent_run (live code). It must NOT be deleted with questioner.py.
  - [x] Move the dataclass definition to build_prefs.py where parse_build_preferences already lives.
  - [x] Update all imports across the codebase: main.py, build_prefs.py, any test files that reference BuildPreferences.
  - [x] Tests: all existing tests that use BuildPreferences still pass; no import of BuildPreferences from questioner remains.

## questioner.py removal

- [x] Delete duplo/questioner.py after BuildPreferences migration
  - [x] After BuildPreferences is moved to build_prefs.py and _first_run is deleted, audit all remaining imports of questioner across the codebase.
  - [x] ask_preferences should have no callers. select_features in the next-phase flow is imported from duplo.selector, not duplo.questioner.
  - [x] Delete duplo/questioner.py and tests/test_questioner.py.
  - [x] Tests: no remaining imports of duplo.questioner anywhere in the codebase; pytest -x passes.

## initializer.py removal

- [x] Evaluate duplo/initializer.py for removal
  - [x] Audit all callers of initializer.create_project_dir and initializer.project_name_from_url.
  - [x] _first_run used initializer to create a target project directory and git init it. Under the new model, the user creates their own project directory and runs duplo init.
  - [x] If project_name_from_url is used by derive_app_name (in saver.py) or another live path, keep only that function and delete the rest.
  - [x] If no remaining callers exist after _first_run removal: delete duplo/initializer.py and tests/test_initializer.py.
  - [x] Tests: no remaining imports of deleted functions.

## _rescrape_product_url legacy fallback

- [x] Remove _rescrape_product_url from duplo/main.py
  - [x] _subsequent_run has two branches for scraping: if spec has scrapeable sources, use _scrape_declared_sources; otherwise fall back to _rescrape_product_url which reads source_url from duplo.json directly.
  - [x] After _first_run removal, no new project can write source_url to duplo.json without SPEC.md. The only projects hitting _rescrape_product_url are pre-migration projects, but those are blocked by the migration gate.
  - [x] Confirm that every path reaching _subsequent_run has a SPEC.md (migration gate guarantees this). If confirmed, delete _rescrape_product_url and its else branch in _subsequent_run.
  - [x] If a path can still reach _subsequent_run without SPEC.md (e.g. .duplo/duplo.json exists but SPEC.md was deleted post-migration), keep the fallback and add a diagnostic instead.
  - [x] Tests: _subsequent_run with a valid SPEC.md never calls _rescrape_product_url; _subsequent_run without SPEC.md either errors or uses the fallback (whichever behavior is chosen).

## Design-data redundancy simplification

- [x] Simplify _detect_and_append_gaps to read design data from SPEC.md only
  - [x] _detect_and_append_gaps currently merges design data from both duplo.json design_requirements AND SPEC.md AUTO-GENERATED block via _merge_design_dicts. Source code comment says "redundant during transition; can simplify in Phase 7."
  - [x] After the new model is the only path, SPEC.md AUTO-GENERATED block is the canonical source. Remove the duplo.json design_requirements merge and read only from spec.design.auto_generated.
  - [x] Remove the _merge_design_dicts call at the gap-detection site. If _merge_design_dicts has no other callers, delete it from gap_detector.py.
  - [x] Tests: gap detection produces the same results when design data is only in SPEC.md; no regression in existing gap-detection tests.

## Scanner heuristics removal

- [x] Remove file-relevance scoring from duplo/scanner.py
  - [x] Phase 5 changed scan_directory to point at ref/ and drop relevance heuristics. Confirm that no legacy scoring code remains (image dimension checks, file size thresholds, etc.).
  - [x] If any legacy scoring functions or constants remain in scanner.py that are no longer called, delete them.
  - [x] Tests: no reference to removed scoring functions; scan_directory works purely on ref/ file inventory.
...
- [x] Remove URL extraction from arbitrary text files in the project directory
  ...
  - [x] Confirm that _subsequent_run reads URLs only from scrapeable_sources(spec), not from file scanning.
...
- [x] Remove any compatibility shims for spec.references and spec.design string access
  - [x] Phase 3 changed spec.references from str to list of ReferenceEntry and spec.design from str to DesignBlock. If any compatibility properties or helper methods were added to ProductSpec to support old-style string access, remove them now.
  ...
  - [x] Tests: no string-access patterns remain; all callers use list of ReferenceEntry and DesignBlock.

## Dead code audit

- [x] Audit duplo/saver.py for dead code
  - [x] Identify functions and constants that were only used by _first_run: save_selections, save_screenshot_feature_map, move_references, write_claude_md, and any others that have no remaining callers after _first_run removal.
  - [x] For each candidate: grep the codebase for all call sites. If only called from _first_run or deleted test code, delete it.
  - [x] Tests: pytest -x passes after deletions.

- [x] Audit duplo/screenshotter.py and duplo/comparator.py for dead code
  - [x] map_screenshots_to_features and save_reference_screenshots in screenshotter.py were used by _init_project. Check for remaining callers.
  - [x] compare_screenshots in comparator.py is used by _compare_with_references in main.py. _compare_with_references is called from _complete_phase which is live code, so comparator.py likely stays.
  - [x] Tests: pytest -x passes after deletions.

- [x] [BATCH] Audit remaining modules for dead code
  - [x] Check duplo/fetcher.py for functions only used by URL-in-text-file scanning.
  - [x] Check duplo/extractor.py for functions only used by the old first-run feature selection flow.
  - [x] Check duplo/collector.py for functions with no remaining callers.
  - [x] Check duplo/notifier.py, duplo/issuer.py, duplo/doc_tables.py for dead exports.
  - [x] For each dead function: delete it and remove its import from any __init__.py or other module.
...
- [x] Add tests/test_phase7_integration.py with test_fresh_directory_without_init_prints_message
...
- [x] Add test_no_dead_imports_remain
...
- [x] Run the full test suite and confirm Phase 7 closes cleanly
  - [x] Execute pytest -x against the duplo repo.
