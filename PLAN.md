...
- [ ] Add `status`, `implemented_in`, and `issues` fields to feature tracking
  - [ ] Each feature dict in `duplo.json` gets two optional fields: `status` (one of `pending`, `implemented`, `partial`) and `implemented_in` (phase label string). New features default to `status: "pending"`.
  - [ ] Add a `save_feature_status(name, status, implemented_in)` function to saver.py that updates a feature by name
...
- [ ] Implement deterministic phase-completion tracking
  ...
  - [ ] For each checked line with a `[feat: ...]` annotation, mark the referenced features as `implemented` with the current phase label
  - [ ] For each checked line with a `[fix: ...]` annotation, mark the corresponding issue as resolved
