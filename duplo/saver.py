...
from duplo.platforms.formatter import format_claude_md_section, format_local_overrides
...
def _format_stack_section(preferences: list[BuildPreferences]) -> str:
    """Build the ``## Stack`` section listing each BuildPreferences entry."""
    lines: list[str] = ["## Stack", ""]
    for i, prefs in enumerate(preferences, start=1):
        platform = prefs.platform or "unspecified"
        language = prefs.language or "unspecified"
        label = f"Stack {i}" if len(preferences) > 1 else "Stack"
        lines.append(f"- {label}: {platform} / {language}")
        for extra in prefs.preferences:
...
def write_claude_md(
    ...
    project_name: str,
    *,
    local_md_content: str = "",
    target_dir: Path | str = ".",
) -> Path:
    """Assemble and write CLAUDE.md to the target project directory.

    The file content is composed of:

    1. A project title heading and an "auto-generated" marker.
    2. A ``## Stack`` section listing platform/language for each
       :class:`BuildPreferences`.
    3. The platform rules section produced by
       :func:`format_claude_md_section` (omitted when *profiles* is empty).
    4. The local overrides section produced by
       :func:`format_local_overrides` (omitted when *local_md_content*
       is blank).

    The file is always overwritten: CLAUDE.md is treated as a derived
    artifact owned by duplo.  Returns the path to the written file.
    """
    root = Path(target_dir)
    root.mkdir(parents=True, exist_ok=True)
    path = (root / CLAUDE_MD_FILENAME).resolve()

    sections: list[str] = [
        f"# {project_name}",
        "",
        "Auto-generated by duplo -- do not edit manually.",
        "",
        _format_stack_section(preferences),
    ]

    platform_section = format_claude_md_section(profiles)
    if platform_section:
        sections.append(platform_section)

    local_section = format_local_overrides(local_md_content)
    if local_section:
